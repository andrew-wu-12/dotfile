...
Several `bin/*.sh` scripts (`checkout-ticket.sh`, `worktree-ticket.sh`, `deploy-one.sh`, `combine-branches.sh`, `deploy-i18n.sh`, `trace-build.sh`) carry `# @raycast.*` header comments, so the same script is both a `~/bin` CLI command and a Raycast Script Command (Raycast points its script-directory setting at `~/bin` and reads these headers to build its UI) — don't remove those comments as dead code.
...
- **`tmux-dev-layout.sh` (`dev [-n|--new-window]`)** — Builds the VSCode-style tmux window (nvim + command + claude) for the current repo/worktree. Idempotent by window name; names windows `{branch}({repo})`.
  - Default overrides the current window in place; `-n`/`--new-window` opens a separate window instead. An existing matching window is always just selected, never duplicated.
  - Session is resolved per repo via `SESSION_GROUP` in `.workspace.conf` (walked `$HOME` → repo root, outer to inner — same file `tmux-window-picker.sh` reads `WORKSPACE_SERVE_CMD` etc. from). No `SESSION_GROUP` anywhere in the ancestry falls back to the repo name as its own session.
  - A target session other than the one attached switches sessions (`ensure_session_and_window`) instead of overriding the current window.
...
- **`tmux-window-picker.sh` (`prefix w`)** — Vertical window switcher, MOP `yarn serve` control, and JIRA ticket picker in one fzf popup. Two row sets toggled by `Tab`; never creates/renames/kills a window itself (`mwt`/`wt`/`wtd` own that lifecycle). Replaces native `choose-tree`; absorbs the removed `prefix n` serve popup and the removed `prefix j` ticket picker.
  - **Active** (default, no JIRA call) — one card per tmux window across all sessions (`session │ 🔴/🟢 name`), with ticket title and, if `WORKSPACE_SERVE_CMD` is set, a serve status line. Workspace/build paths resolve per pane by walking up from `pane_current_path` for a `.workspace.conf` (`workspace_config_load`).
  - **All** (`Tab`) — adds a card per JIRA ticket assigned to you with no worktree yet (JQL against the MOP board, cached 5 min, refetched on `Tab` or when stale). A ticket that already has a worktree shows as a window card instead.
  - `enter` — switches to a window card; creates the worktree + window (`worktree-ticket.sh -n`) for a ticket card.
  - `ctrl-s`/`ctrl-r`/`ctrl-x`/`ctrl-v` — set/restart/stop/view-log the hidden `serve(mop-console-monorepo)` window (no-op on a ticket card).
  - `ctrl-g` — traces the highlighted worktree's CI jobs (any repo with `.tmux-build.conf`); exits the popup.
  - `ctrl-p` — deploys the highlighted ticket (`deploy_run()`, MOP/Jenkins-only): pick a branch, then job(s), then trace inline in the same pane.
  - `ctrl-b` — opens the highlighted ticket in the JIRA browser (ticket cards only).
  - `ctrl-n` — opens `NOTE_PATH` in an nvim popup.
  - `ctrl-l` — busts the ticket-status preview cache and the JIRA ticket-list cache.
  - Preview pane (right 60%) runs the workspace's `WORKSPACE_PREVIEW_CMD`; fetches live and uncached per highlighted row (caching planned — see `tmux-ticket-status.sh`).
...
- **`tmux-build-backend-jenkins.sh`** — Jenkins backend for the generic engine: `jenkins_find_build`/`jenkins_poll_build`, dispatched by name (`trace_backend_find_build`/`trace_backend_poll_build` in `tmux-build-trace-lib.sh` call `"${BUILD_BACKEND}_find_build"`/`"_poll_build"`) when a worktree's config sets `BUILD_BACKEND=jenkins`. Same Jenkins REST + `jq` queries the old MOP-only code used, just parameterized on `$BUILD_JENKINS_URL` (from config) and `$JENKINS_TOKEN` (fetched by the caller via `init-lib.sh`'s `cred_find "$BUILD_CRED_NAME"` — replacing the old zsh-shell-out-to-`.zshrc` token fetch, since a configurable credential name can't hang off one fixed env var). A non-Jenkins backend is a sibling file defining `<backend>_find_build`/`<backend>_poll_build` with the same JSON shape (`{number,url,result,timestamp,estimatedDuration,duration}` / `{result,building,estimatedDuration}`) — `trace_run()` doesn't know or care which backend produced them. A backend with no duration estimate renders "building... (Elapsed: …)" instead of a %-bar + ETA, since `trace_run()` only draws the bar when `estimatedDuration` is present and positive.
- **`tmux-build-backend-forgejo.sh`** — Forgejo backend for the generic engine (`BUILD_BACKEND=forgejo`), used by the `homelab` repo. Unlike Jenkins (one job = one triggerable pipeline), a Forgejo repo's CI is the combined commit-status of a ref — every workflow job posts its own status entry and the repo's commit-status API already aggregates them — so the `BUILD_JOBS` line's job field is accepted but unused; there is exactly one thing to trace per ref, not one per workflow file. Needs `BUILD_FORGEJO_URL` + `BUILD_FORGEJO_REPO` from config and a token via `cred_find "$BUILD_CRED_NAME"`. Branch names containing `/` must be percent-encoded before hitting the commits/status endpoint, or Forgejo's router 404s by silently mis-splitting the path.
...
- **`tmux-forgejo-picker.sh` (`prefix i`)** — Issue picker for the `homelab` repo (Forgejo, not JIRA/MOP): fzf popup of open issues on `andrew_wu/homelab`, cached 5 min. `enter` creates/opens the issue's worktree via `wt -n` (branch `feature/<number>-<slug>`, plain `worktree-generic.sh`, no JIRA/PR machinery); `ctrl-o` opens it in the browser; `ctrl-r` busts the cache. Mirrors `tmux-window-picker.sh`'s structure but is entirely separate — homelab tickets never appear in the JIRA-driven `prefix w` picker, and vice versa.
