...
- **`tmux-window-picker.sh` (`prefix w`)** — Vertical window switcher and MOP `yarn serve` control, merged into one fzf popup: a card per tmux window across all sessions (`session │ 🔴/🟢 name`), each also carrying its ticket title (if any) and, for MOP worktree windows, a serve status/marker line. Enter switches (across sessions via `switch-client` + `select-window`) and exits; `ctrl-s`/`ctrl-r`/`ctrl-x`/`ctrl-v` set/restart/stop/view-log the single hidden `serve(mop-console-monorepo)` window without switching, looping back into a refreshed picker instead of exiting; `ctrl-g` exits this popup and, a beat later, opens a new tmux popup running a live Jenkins build trace for the highlighted card's worktree (see below); Esc cancels. Never creates/renames/kills a *window* itself — that lifecycle stays with `mwt`/`wt`/`wtd`; only the hidden serve window's process/target is mutated by `ctrl-s`/`r`/`x`/`v`, and the trace popup is ephemeral (no window). Replaces native `choose-tree`; styled to catppuccin mocha. Absorbs what used to be a separate `prefix n` popup (`tmux-serve-popup.sh`, now removed) — see [Parallel Ticket Workspaces](#parallel-ticket-workspaces-git-worktrees). Right 60% is an fzf `--preview` running `tmux-ticket-status.sh --print` against the highlighted card's worktree path (blank/placeholder for non-MOP windows); popup widened to 90%/80% to fit it. fzf re-invokes the preview on every highlighted row (including transient ones while scrolling), so this is a live, uncached ticket-status fetch per cursor move — a caching layer is the planned follow-up, see `tmux-ticket-status.sh`.
- **`tmux-build-trace-lib.sh`** — Shared build-trace helpers (`trace_vpn_connected`, `trace_parse_branch`, `trace_resolve_uat_branch`, `trace_open_popup`) sourced by `tmux-window-picker.sh` (`ctrl-g`), bash-3.2-clean like `tmux-serve-lib.sh`. `trace_open_popup` VPN-gates (same `scutil --nc list` check as `deploy-one.sh`/`tmux-ticket-status.sh`), resolves the ticket's uat/epic branch (shelling out to zsh for `ticket-lib.sh`'s `get_ticket_parent`, since that helper is zsh-only), then schedules `tmux display-popup -E -w 90% -h 80%` running `tmux-build-trace.sh` via `tmux run-shell -b` with a short delay — a tmux client only shows one popup at a time, and the picker is already running inside one (`bind w display-popup ...` in `.tmux.conf`), so the new popup can't be opened directly from inside the old one; scheduling it with a beat's delay lets the picker's own popup close first instead of the two conflicting. An earlier version ran the trace as a detached background process with a per-card cache-file status line, then briefly as a plain `new-window`, before landing on this popup approach.
- **`tmux-build-trace.sh`** — The live-trace popup `tmux-build-trace-lib.sh` opens; not run directly. Renders the same progress-bar UX as `trace-build.sh` (search, then poll every 2s with an in-place redraw, then an `osascript` notification), extended to 4 fixed jobs instead of a dynamic branch search: `mop_console_monorepo_feature`/`_dev` against the ticket's own `feature/`/`hotfix/` branch, `mop_console_monorepo_epic_or_hotfix`/`_uat` against `uat/<parent>` (or the hotfix branch itself if there's no parent). Ends by `exec`-ing into an interactive shell so the popup stays open showing the final results until you close it yourself, instead of vanishing the moment the script exits (the popup's own `-E` flag closes it once that shell exits).
