# Shared build-trace engine: discover, poll, and render progress for one or
# more CI builds until they finish. Backend-agnostic — dispatches to
# "${BUILD_BACKEND}_find_build" / "${BUILD_BACKEND}_poll_build" (e.g.
# tmux-build-backend-jenkins.sh's jenkins_find_build/jenkins_poll_build),
# resolved per-worktree by tmux-build-config.sh's build_config_load. Bash
# 3.2-clean.
...
# True if VPN is connected. Only meaningful for repos whose config sets
# BUILD_VPN_CHECK=1 (MOP's Jenkins is only reachable over VPN — a repo using
# a publicly-reachable CI backend doesn't need this gate at all).
trace_vpn_connected() {
  scutil --nc list 2>/dev/null | grep -q "Connected"
}

# Calls "${BUILD_BACKEND}_find_build"/"_poll_build" if the backend module
# defines it, so a misconfigured/unsupported BUILD_BACKEND fails a single
# job spec quietly (it just never shows up as tracked) instead of erroring.
...
# returns nonzero if no .tmux-build.conf resolves for this worktree, or VPN
# is required (BUILD_VPN_CHECK=1) and down.
trace_open_popup() {
...
# Discovers, polls until done, and renders one or more CI jobs in place;
# prints a summary and fires a completion osascript notification. Does not
# exec into a shell.
#
# Args: one or more "KEY|JOB|BRANCH|LABEL" specs. Requires $BUILD_BACKEND set
# (by build_config_load) and any backend-specific env it needs (e.g.
# $JENKINS_TOKEN for the jenkins backend). $TRACE_NOTIFY_SUBTITLE (optional)
# sets the notification subtitle.
#
# Returns 1 if no spec ever found a matching build (retries discovery for
# ~16s, since a just-triggered build sits in the CI system's queue briefly
# before appearing); 0 if every tracked build succeeded; 2 if at least one
# didn't.
trace_run() {
      ...
      if [ "$result" = "null" ] || [ "$result" = "BUILDING" ]; then
        ...
        if [ "$building" = "false" ]; then
          ...
          if [ -n "$est" ] && [ "$est" != "null" ] && [ "$est" -gt 0 ] 2>/dev/null; then
            ...
            # No duration estimate available from this backend (e.g. GitHub
            # Actions) — show elapsed time instead of faking a percentage.
