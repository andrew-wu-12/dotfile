...
description: "Rebuild a disposable integration branch by merging several feature branches into one base. Use whenever the user wants to combine/merge multiple feature branches into one deployable branch, show several features under one deploy URL, build an integration branch, or 'combine branches MOP-X MOP-Y'."
---
...
4. On **success**: report the combined branch SHA. If the user wants it
   deployed, use [deploy-one](../deploy-one/SKILL.md) on the integration branch.
...
```bash
# Combine three features into one integration branch
~/bin/combine-branches.sh -n integration/MOP-27675 \
  feature/MOP-27675 feature/MOP-27676 feature/MOP-27677

# Build a combined branch off a uat base
